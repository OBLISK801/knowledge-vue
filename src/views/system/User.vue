    ...
    <el-card>
      <el-form :inline="true" ref="form" :model="queryMap" label-width="70px" size="small">
        <el-form-item label="用户名">
          <el-input @keyup.enter.native="searchUser" @clear="searchUser" clearable
...
export default {
  ...
  methods: {
    //加载用户列表
    getUserList () {
      this.$http.get('system/user/findUserList', { params: this.queryMap }).then(res => {
     ...
     */
    changUserStatus (row) {
     this.$http.put('system/user/updateStatus/' + row.id + '/' + row.status).then(res => {
       if (res.data.code === 20000) {
     ...
     */
    edit (id) {
      this.$http.get('system/user/edit/' + id).then(res => {
        if (res.data.code === 20000) {
    ...
    assignRoles (id) {
      ...
      this.$http.get('system/user/' + id + '/roles').then(res => {
        if (res.data.code === 20000) {
    ...
    addUser () {
      this.$refs.addFormRef.validate(valid => {
        if (!valid) {
          ...
          this.$http.post('system/user/add', this.addForm).then(res => {
            if (res.data.code === 20000) {
              this.$message.success({
                title: '操作成功',
                message: '用户添加成功',
    ...
    updateUser() {
      this.$refs.editFormRef.validate(valid => {
        if (!valid) {
          ...
          this.btnLoading = true;
          this.btnDisabled = true;
          this.$http.put("system/user/update/" + this.editForm.id, this.editForm).then(res => {
            if (res.data.code === 20000) {
              this.$message({
                title: "操作成功",
                message: "用户基本信息已更新",
              ...
              return this.$message.error("用户信息更新失败:" + res.data.errorMsg);
            }
          }).catch()
    ...
    doAssignRoles() {
      ...
      this.btnLoading = true;
      this.btnDisabled = true;
      this.$http.post("system/user/" + this.uid + "/assignRoles", this.value).then(res => {
        if (res.data.code === 20000) {
          this.$message.success({
            title:'操作成功',
            message:'用户分配角色成功',
