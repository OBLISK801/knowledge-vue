<template>
  <div>
    ...
    <el-card>
      ...
      <el-row :gutter="20">
        <el-col :span="8">
          <el-input clearable size="small" placeholder="请输入角色名查询" v-model="queryMap.roleName" class="input-with-select"
                    @clear="getRoleList">
            <el-button slot="append" icon="el-icon-search" @click="search"></el-button>
          </el-input>
        </el-col>
        <el-col :span="2">
          <el-button size="small" v-hasPermission="'role:add'" type="success" icon="el-icon-circle-plus-outline"
      ...
      <!-- 表格区域 -->
      <template>
        <el-table v-loading="loading" :data="roleData" border style="width: 100%;margin-top:20px;" height="470"
                  size="small">
          ...
          <el-table-column fixed="right" label="操作" width="200">
            <template slot-scope="scope">
              ...
              <el-button @click="edit(scope.row.id)" v-hasPermission="'role:edit'" type="text" icon="el-icon-edit"
                         size="small">编辑
              </el-button>
              <el-button v-hasPermission="'role:delete'" @click="del(scope.row.id)" type="text" icon="el-icon-delete"
                         size="small">删除
              </el-button>
            </template>
          </el-table-column>
      ...
      <el-dialog title="分配菜单权限" :visible.sync="grantDialogVisible" width="38%">
        ...
        <span slot="footer" class="dialog-footer">
          ...
          <el-button v-hasPermission="'role:authority'" type="primary" icon="el-icon-setting" @click="authority"
                     :loading="btnLoading" :disabled="btnDisabled">授 权</el-button>
        </span>
      </el-dialog>
    </el-card>
  </div>
...
export default {
  ...
  methods: {
    //加载用户列表
    getRoleList () {
      this.$http.get('system/role/findRoleList', { params: this.queryMap }).then(res => {
        console.log(res.data)
        if (res.data.code === 20000) {
          this.roleData = res.data.data.results
    ...
    changRoleStatus (row) {
      this.$http.put(
        'system/role/updateStatus/' + row.id + '/' + row.status).then(res => {
        if (res.data.code === 20000) {
    ...
    grant (id) {
      //加载所有菜单以及用户拥有的菜单权限id
      this.$http.get('system/role/findRoleMenu/' + id).then(res => {
        if (res.data.code === 20000) {
          //默认选中的树的数据
          let that = this
          setTimeout(function () {
            res.data.data.mids.forEach(value => {
    ...
    //编辑
    edit (id) {
      this.$http.get('system/role/edit/' + id).then(res => {
        if (res.data.code === 20000) {
